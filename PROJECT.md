...
- `IMPORT_LINKEDIN_PROFILE` (side panel $\rightarrow$ background) $\rightarrow$ opens `https://www.linkedin.com/in/me/`, waits for load (20 s), polls the tab with `SCRAPE_LINKEDIN_PROFILE` up to 6× at 1.5 s; returns `{ success: boolean, profile?: ProfileImport, error?: string, tabId?: number }`. Login/authwall pages return "Please sign in to LinkedIn, then try again.". The tab is never closed.
- `SCRAPE_LINKEDIN_PROFILE` (background $\rightarrow$ content script) $\rightarrow$ `{ success: boolean, profile?: ProfileImport, error?: string }`; errors when the tab is not a `linkedin.com/in/...` page or the profile has not rendered yet.
