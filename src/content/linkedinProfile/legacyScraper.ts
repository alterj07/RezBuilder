/**
 * LEGACY LinkedIn layout scraper (pre-2026): `#experience` anchor divs,
 * `<h1>` name, `[aria-hidden="true"]` / `.visually-hidden` duplicated spans and
 * `<ul><li>` entries. LinkedIn no longer renders this DOM, but the code is kept
 * as a fallback for the new scraper when its own selectors find nothing.
 *
...
import { isLinkedInProfileUrl } from './pageKind';
...
export function inferDegreeLevelLegacy(text: string): DegreeLevel {
  const raw = text || '';
...
function parseEducationEntry(li: Element, currentYear: number): Omit<ProfileEducation, 'id'> | null {
  ...
  if (degreeText) {
    ...
    education.degreeLevel = inferDegreeLevelLegacy(degreeText);
  }
...
/**
 * Scrapes a LEGACY-layout LinkedIn profile page (or one of its `/details/*`
 * sub-pages) into a partial profile. Never throws.
 */
export function scrapeLinkedInProfileLegacy(document: Document, url: string): ProfileImport {
